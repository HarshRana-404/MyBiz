...
class AnalysisFragment : Fragment() {
    ...
    private fun loadOptimizedData() {
        try {
            ...
            var monthKey = Constants.getYearMonthForSales(Constants.getDateTime()) // e.g. "2025_10"
            ...
            monthKey = monthKey.replace("_","-")
            monthKey = monthKey.replace("_","-")

            // ✅ Monthly Stock (Already stored by month)
            val monthlyStock = Constants.alStocksCached
                .filter { it.date.contains(monthKey) }
                .sumOf { it.price }
            ...
            val mostSold = hmMostSold.maxByOrNull { it.value }?.key ?: "No Sales"
            tvMostSoldProduct.text = mostSold
        } catch (ex: Exception) {
    ...
    private fun calculateSalesBetween() {
        ...
        hmProductQuantity.forEach { (productName, qty) ->
            val product = Constants.alProductsOptimized.find { it.name == productName }!!
        ...
        tvSalesBetweenTotal.text = "₹ $total"
    ...
    private fun dateToMillis(dateString: String): Long {
        // dateString like "sales_2025_10_24"
        val cleaned = dateString.replace("sales_", "").replace("_", "-")
        val sdf = SimpleDateFormat("yyyy-MM-dd", Locale.getDefault())
        return sdf.parse(cleaned)?.time ?: 0
    }
}
